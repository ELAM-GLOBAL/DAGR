...
import { SITES, Site } from '../data/phoenixEbra';
import { useDagrTheme } from './ThemeContext';

/**
 * Operator-console US site map for the Phoenix Ebra workspace.
...
/* Continental US outline — hand-traced polygon, ~50 points. */
const US_OUTLINE_POINTS = [
    [-123, 48.5], [-124, 46.5], [-124, 41], [-122.5, 38], [-121, 35.5],
    [-118.5, 34], [-117.2, 32.7], [-114.7, 32.7], [-111, 31.5], [-108, 32],
    [-106.5, 31.8], [-104.6, 30.3], [-102, 29.5], [-99.5, 27.5], [-97.4, 26],
    [-95.5, 29], [-93.8, 29.7], [-91, 29.2], [-89, 30.4], [-87.8, 30.3],
    [-85.5, 30], [-83.5, 29.9], [-82.7, 27.9], [-81.2, 25.8], [-80.1, 25.7],
    [-80, 27], [-81, 30.5], [-80.9, 32], [-79.9, 32.8], [-78.9, 33.7],
    [-76.5, 35.3], [-75.5, 36.8], [-75.1, 38.3], [-74, 40.6], [-71.8, 41.3],
    [-71.1, 42.3], [-70.7, 43.5], [-68, 44.3], [-67.1, 45], [-69, 47],
    [-71.4, 45], [-74.7, 45], [-78, 43.5], [-83, 41.6], [-84, 46],
    [-87.6, 45.9], [-92, 47], [-95, 49], [-104, 49], [-117, 49],
    [-122.5, 48.5],
];

const usOutlinePath = US_OUTLINE_POINTS
    .map(([lng, lat], i) => {
        ...
        <div style={{
            ...
            <div style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                    <div>
                        <div style={{
                            ...
                            FACILITY OPS · CONTINENTAL US · 5 NODES
                        </div>
                        <h3 style={{
                ...
                <div style={{
                    ...
                    <span>PROJECTION · EQUIRECTANGULAR · ALBERS-ALIGNED</span>
                </div>
            </div>
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <Metric palette={p} label="Chambers" value={`${site.chambers}`} />
                <Metric palette={p} label="Plantlets / mo" value={`${(site.monthlyVolume / 1000).toFixed(1)}K`} />
                <Metric palette={p} label="Order-to-Ship" value={`${site.tatDays}d`} />
                <Metric palette={p} label="Contamination" value={`${site.contamination}%`} />
                <Metric palette={p} label="Margin" value={`${site.margin}%`} accent />
                <Metric palette={p} label="Cultivars" value={`${site.cultivars.length}`} />
            </div>
